...
template <typename T> void convert(cv::Mat &m, std::map<T, T> conv);
template <typename T> void convert(cv::Mat &m, T from, T to);

// Bucket fill image using a specified conversion map.
// Notes:
//  - Currently only supports 4-way fills (ie., not diagonal, 8-way fills)
template <typename T> void fill(cv::Mat &m, std::map<T, T> conv, int R, int C);
...
template <typename T> void fill(cv::Mat &m, std::map<T, T> conv, int R, int C) {
  ...
  // queue of all filled-in pixels whose neighbours are yet to be checked
  ...
  while (!q.empty()) {
    ...
    if (r > 0)
      adj.push_back(std::make_pair(r - 1, c));
    if (c > 0)
      adj.push_back(std::make_pair(r, c - 1));
    if (r < m.rows - 1)
      adj.push_back(std::make_pair(r + 1, c));
    if (c < m.cols - 1)
      adj.push_back(std::make_pair(r, c + 1));
