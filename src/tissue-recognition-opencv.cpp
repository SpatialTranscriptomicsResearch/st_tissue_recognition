...
void recognize_tissue(const Mat &img, Mat &msk, Mat &user_annotation,
    ...
    if(user_annotation.size() == img.size()) {
        for(auto it = user_annotation.begin<uint8_t>();
                it != user_annotation.end<uint8_t>();
                ++it) {
            switch(*it) {
                case Annotation::FGD: 
...
void init_msk(const Mat &img, Mat &msk) {
    ...
    for(auto it = msk.begin<uint8_t>();
            it != msk.end<uint8_t>(); ++it)
...
static void fill_fgd_holes(Mat &msk) {
    ...
    convert<uint8_t>(msk,
            std::map<uint8_t,uint8_t> {
                { GC_PR_BGD, flag_pr_bgd },
                { GC_BGD, flag_bgd }
            });

    for(int r=0, c=0, dr=1, dc=0; c>=0; r+=dr, c+=dc) {
        if(c==0 && r==msk.rows-1) { dr=0; dc=1; }
        if(c==msk.cols-1 && r==msk.rows-1) { dr=-1; dc=0; }
        if(c==msk.cols-1 && r==0) { dr=0; dc=-1; }

        fill<uint8_t>(msk,
                std::map<uint8_t, uint8_t> {
                    { flag_pr_bgd, GC_PR_BGD },
                    { flag_bgd, GC_BGD }
                },
                r, c);
    }
...
static void remove_small_components(Mat &msk, double threshold) {
    ...
    convert(labels, map<uint8_t,uint8_t> {
            { GC_PR_BGD, GC_BGD },
            { GC_PR_FGD, GC_FGD }
            });
    ...
    for(int r = 0, d = 1; r < msk.rows; ++r, d = -d) {
        for(int c = d > 0 ? 0 : msk.cols-1; c >= 0 && c < msk.cols; c += d) {
            ...
            if(small[labels.at<short>(r,c)]) {
                if(large_component != -1) cur_pixel = large_component;
                else unknown.push_back(&cur_pixel);
            }
            else {
                if(large_component == -1)
                    for(auto p : unknown) *p = cur_pixel;
