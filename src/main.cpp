...
static void help() {
    cout << "\nThis program tries to infer tissue and surrounding in images of "
        ...
        "Usage:\n"
        "tissue-recognition <image_name> [<scale>], default is scale = 1.0\n"
        GIT_DESCRIPTION << "\n"
        << endl;
}

static const unsigned short radius = 10;
static const map<int, Scalar> colors{
    {Annotation::FGD, Scalar(0, 255, 0)},
...
int main(int argc, char **argv) {
    ...
    std::cout << "done. (resized from " << orig_cols << "x" << orig_rows
              << " to " << img.cols << "x" << img.rows << ".)" << std::endl;
